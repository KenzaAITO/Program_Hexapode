...
def camera_thread_function():
    ...
    cap = cv2.VideoCapture(
        0, cv2.CAP_V4L2
    )  # Utiliser l'index approprié pour votre caméra et V4L2 backend
...
try:
    ...
    for dxl_id in LIFT_SERVOS + ROTATE_SERVOS:
        ...
        if dxl_comm_result != COMM_SUCCESS:
            print(
                f"Error disabling torque for Dynamixel {dxl_id}: {packetHandler.getTxRxResult(dxl_comm_result)}"
            )
        elif dxl_error != 0:
            print(
                f"Error disabling torque for Dynamixel {dxl_id}: {packetHandler.getRxPacketError(dxl_error)}"
            )
